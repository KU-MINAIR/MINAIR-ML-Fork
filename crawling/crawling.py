            ...
            if (all_post_link.count(post_link) == 0):
                ...
                if blog_m:
                    ...
                    if blog_text:
                        ...
                        print(str(n * len(add_word) * len(posts) + i * len(posts) + j + 1) + "/" + str(
                            (len(query) - 1) * len(add_word) * len(posts)) + " complete...")
                    else:
                        print(str(n * len(add_word) * len(posts) + i * len(posts) + j + 1) + "/" + str(
                            (len(query) - 1) * len(add_word) * len(posts)) + " .....fail")

            else:
                print(str(n * len(add_word) * len(posts) + i * len(posts) + j + 1) + "/" + str(
                    (len(query) - 1) * len(add_word) * len(posts)) + " ..overlap..")
            print("-" * 50)
